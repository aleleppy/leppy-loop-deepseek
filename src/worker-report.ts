...
export function parseWorkerReport(output: string): WorkerReport {
  ...
  if (report.status === 'completed' && report.validation.status !== 'passed') throw new Error('completed worker outcome requires passed validation evidence')
  ...
  if (report.status === 'completed' && contradictoryFailure) throw new Error(`completed worker outcome contradicts explicit failure: ${contradictoryFailure.slice(0, 512)}`)
  return report
}
