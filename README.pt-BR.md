...
Runs Web terminam localmente por padrão. Somente linguagem humana explícita como `/leppy-loop continuar e publicar quando tudo passar` adiciona publicação remota ao grant one-shot. O modelo não pode acrescentar nem repetir essa autoridade, e workers continuam sem push ou `gh`. Instale e autentique o GitHub CLI (`gh auth status`) antes de optar pela publicação.
