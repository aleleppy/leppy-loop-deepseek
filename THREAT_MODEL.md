...
- Gate command fingerprint is denied in workers. Gates never auto-retry after failure/crash. A repeated fingerprint requires a direct slash/CLI invocation with `--retry-gate` or `--repair-gate` plus the exact authenticated run ID; the model-facing tool has neither field, and every attempt receives a durable receipt. Repair fails closed on a dirty worktree, reopens only the immediately preceding completed closure through a controller commit, and gives a bounded prior receipt to a fresh worker under that closure's existing path scope.
...
The Harness filesystem sandbox does not confine network. Existing repository scripts may access network, invoke native code, exploit the OS account, or hide behavior behind an otherwise permitted test command. The v0.2.22 boundary assumes the repository and its existing scripts are trusted.
