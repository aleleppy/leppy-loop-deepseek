...
3. Each ordinary line gets a fresh worker process and SDK session. The controller treats report format, validation, and commit ceremony as advisory; it reconciles safe in-scope Git effects and marks the row unless implementation is explicitly impossible.
4. Closure follows the same policy and may be a clean no-op. Out-of-scope generator effects are restored; in-scope repairs are adopted automatically.
5. Gate has no ordinary worker. The controller executes the opaque command, writes a receipt, and on failure automatically reopens the adjacent closure for bounded repair before retrying the unchanged gate fingerprint. At the limit it HMAC-binds the receipt, removes transient effects including ignored artifacts, and advances with advisory evidence; publication always reruns its strict final gate.
...
Each line starts an independent context, so shared conversational cache is intentionally lost and model cost may be higher. Version `0.3.48` supports only the tested Harness pin. Network confinement, automatic push, PR mutation, release publication, package publication, and deployment are not provided. No remote action is automatic.
